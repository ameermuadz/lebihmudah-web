...
import PropertyOwnerEditor from "@/components/PropertyOwnerEditor";
...
export default async function PropertyDetailPage({
  ...
  const isOwnerSession = session?.user.role === "OWNER";
  const isPropertyOwner =
    isOwnerSession && session.user.id === property.ownerId;
  const backHref = isOwnerSession ? "/dashboard" : "/";
  const backLabel = isOwnerSession ? "Back to dashboard" : "Back to search";
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {isPropertyOwner ? (
          <div className="space-y-6">
            ...
            <PropertyOwnerEditor property={property} />
          </div>
        ) : (
        ...
        {!isPropertyOwner && isOwnerSession ? (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-100">
            Owner accounts can edit their own properties from this page. Use the
            dashboard to review bookings for your listings.
          </section>
        ) : null}
      </div>
    </main>
